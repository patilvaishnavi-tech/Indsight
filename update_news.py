...
Uses gemini-1.5-flash — verified working, fast, free tier
FIXED: Correct container IDs matching actual HTML
"""
...
{"stories":[{"tag":"TAG","headline":"Real headline with company + number","summary":["What happened: specific real event","Why it happened: cause","Key number: exact figure","India impact: market effect","What to watch: next development"],"source":"Publication","url":"https://publication.com"}]}
Exactly 5 real stories from last 30 days. JSON only."""

# CORRECT container IDs matching actual HTML divs
SECTIONS = [
  ("anews",
   f"Senior auto journalist India {TODAY}. TOP 5 real auto news — ICE, EV, hydrogen, safety, exports. Companies: Maruti, Tata, M&M, Hero, Bajaj, TVS, Ola Electric, Ather. Sources: ET Auto, Autocar India. {NEWS_FORMAT}",
   {"ICE":"#1d4ed8","EV":"#15803d","Hydrogen":"#6d28d9","2W":"#1d4ed8","4W":"#1d4ed8","CV":"#1d4ed8","Policy":"#155e75","Safety":"#1d4ed8","Export":"#991b1b"}),

  ("enews",
   f"Senior energy analyst India {TODAY}. TOP 5 real energy news — solar, wind, BESS, hydrogen, grid. Companies: NTPC, Adani Green, Tata Power, Waaree, Suzlon. Sources: Mercom India, ET Energy. {NEWS_FORMAT}",
   {"Solar":"#854d0e","Wind":"#1e40af","BESS":"#6d28d9","Hydrogen":"#6d28d9","Grid":"#14532d","Gas":"#14532d","Coal":"#92400e"}),

  ("policy",
   f"Energy policy analyst India {TODAY}. TOP 5 real policy news — SECI tenders, PLI solar/batteries, PM-KUSUM, FAME III, ALMM. Sources: MNRE, PIB, ET Energy. {NEWS_FORMAT}",
   {"PLI":"#155e75","FAME":"#155e75","SECI":"#854d0e","NHGM":"#6d28d9","Tender":"#155e75","Budget":"#155e75"}),

  ("lead",
   f"Business journalist auto CXO moves {TODAY}. TOP 5 real leadership changes — CEO, CFO, MD, Board at India + global auto. Sources: ET Auto, Business Standard. {NEWS_FORMAT}",
   {"Leadership":"#92400e","CEO":"#92400e","Board":"#92400e","Appointment":"#92400e","Exit":"#92400e"}),

  ("inv",
   f"Auto investment analyst India {TODAY}. TOP 5 real investment/M&A/JV/PLI news — plant expansions, EV deals, battery investments. Sources: ET Auto, Reuters, PIB. {NEWS_FORMAT}",
   {"Capex":"#991b1b","M&A":"#991b1b","JV":"#991b1b","PLI":"#155e75","EV Infra":"#15803d","Global":"#991b1b"}),

  ("liionnews",
   f"Battery analyst {TODAY}. TOP 5 real Li-ion news — CATL, BYD, LG ES, India PLI (Exide, Amara Raja), cell pricing LFP vs NMC. Sources: BloombergNEF, Reuters, ET. {NEWS_FORMAT}",
   {"CATL":"#6d28d9","BYD":"#6d28d9","LG ES":"#6d28d9","India PLI":"#155e75","Cell Price":"#6d28d9","Solid State":"#6d28d9"}),

  ("tradenews",
   f"Trade analyst India {TODAY}. TOP 5 real import/export policy news — DGFT, FTA (UAE, UK, EU), anti-dumping duties, RoDTEP. Sources: DGFT, PIB, ET. {NEWS_FORMAT}",
...
def inject(html, cid, stories, tag_map):
    ...
    banner = (
        ...
        f'Click Read Original to verify</div>'
    )
    cards = banner + '<div class="news-grid">' + "".join(
        render_card(s,i,len(stories),tag_map) for i,s in enumerate(stories)
    ) + "</div>"

    # Match the exact div and replace its inner content
    pattern = rf'(<div id="cont-{re.escape(cid)}">)(.*?)(</div>)'
    new_html = re.sub(pattern, rf'\g<1>{cards}\g<3>', html, flags=re.DOTALL)

    if new_html == html:
        print(f"   ⚠  Container 'cont-{cid}' not found in HTML!")
    else:
...
def main():
    ...
    with open(HTML_FILE,"w",encoding="utf-8") as f:
        f.write(html)
    print(f"\n✅ Saved — {total} stories across {len(SECTIONS)} sections")

    if AUTO_GIT_PUSH:
        print("\n🚀 Pushing to GitHub...")
        os.system(f'git add {HTML_FILE}')
        code = os.system(f'git commit -m "Daily news: {TODAY}" && git push origin main')
        print("✅ Done!" if code==0 else "⚠ Git push failed")

    print(f"\n{'═'*52}\n  Complete! {total} stories injected.\n{'═'*52}\n")
